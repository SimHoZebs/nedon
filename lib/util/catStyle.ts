const catStyleArray: {
  [key: string]: {
    icon: string;
    bgColor: string;
    textColor: string;
    border: string;
  };
} = {
  Travel: {
    icon: "icon-[mdi--car-outline]",
    bgColor: "bg-purple-300",
    textColor: "text-purple-300",
    border: "border border-1 border-purple-300",
  },
  Taxi: {
    icon: "icon-[bx--taxi]",
    bgColor: "bg-yellow-300",
    textColor: "text-yellow-300",
    border: "border border-1 border-yellow-300",
  },
  "Fast Food": {
    icon: "icon-[ion--fast-food-outline]",
    bgColor: "bg-red-300",
    textColor: "text-red-300",
    border: "border border-1 border-red-300",
  },
  "Food and Drink": {
    icon: "icon-[fluent--food-24-regular]",
    bgColor: "bg-yellow-300",
    textColor: "text-yellow-300",
    border: "border border-1 border-yellow-300",
  },
  "Airlines and Aviation Services": {
    icon: "icon-[lucide--plane-takeoff]",
    bgColor: "bg-teal-300",
    textColor: "text-teal-300",
    border: "border border-1 border-teal-300",
  },
  "Coffee Shop": {
    icon: "icon-[ci--coffee]",
    bgColor: "bg-orange-300",
    textColor: "text-orange-300",
    border: "border border-1 border-orange-300",
  },
  Shops: {
    icon: "icon-[mdi--shopping-outline]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  Payment: {
    icon: "icon-[mdi--exchange]",
    bgColor: "bg-zinc-300",
    textColor: "text-zinc-300",
    border: "border border-1 border-zinc-300",
  },
  Transfer: {
    icon: "icon-[mdi--exchange]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  "Gyms and Fitness Centers": {
    icon: "icon-[mingcute--fitness-line]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  Recreation: {
    icon: "icon-[material-symbols--relax-outline]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  Overdraft: {
    icon: "icon-[ph--hand-coins-bold]",
    bgColor: "bg-red-300",
    textColor: "text-red-300",
    border: "border border-1 border-red-300",
  },
  Rent: {
    icon: "icon-[tabler--home-dollar]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  Deposit: {
    icon: "icon-[mdi--instant-deposit]",
    bgColor: "bg-zinc-300",
    textColor: "text-zinc-300",
    border: "border border-1 border-zinc-300",
  },
  Restaurants: {
    icon: "icon-[material-symbols--food-bank-outline-rounded]",
    bgColor: "bg-yellow-300",
    textColor: "text-yellow-300",
    border: "border border-1 border-yellow-300",
  },
  "Credit Card": {
    icon: "icon-[mdi--credit-card-outline]",
    bgColor: "bg-yellow-300",
    textColor: "text-yellow-300",
    border: "border border-1 border-yellow-300",
  },
  Credit: {
    icon: "icon-[mdi--bank-outline]",
    bgColor: "bg-orange-300",
    textColor: "text-orange-300",
    border: "border border-1 border-orange-300",
  },
  Health: {
    icon: "icon-[akar-icons--health]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  "Bank Fees": {
    icon: "icon-[mingcute--bank-line]",
    bgColor: "bg-green-300",
    textColor: "text-green-300",
    border: "border border-1 border-green-300",
  },
  Unknown: {
    icon: "icon-[mdi--shape-outline]",
    bgColor: "bg-zinc-400",
    textColor: "text-zinc-400",
    border: "border border-1 border-zinc-400",
  },
};

export default catStyleArray;

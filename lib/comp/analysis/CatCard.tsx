...
const CatCard = (props: Props) => {
  const totalAmount = new Decimal(
    props.cat.primary.total.toString(),
  ).absoluteValue();
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-end">
            ...
            {props.catSettings &&
              !new Decimal(props.catSettings.budget).isZero() && (
                <p className="text-sm text-zinc-400">
                  {totalAmount
                    .div(props.catSettings.budget)
                    .mul(100)
                    .toNumber()}
                  % of {new Decimal(props.catSettings.budget).toNumber()}
                </p>
              )}
          </div>
        </div>
